...
class Raw_Frame_Archive_Service(Service):
    ...
    async def reconfigure(self, topic, message, reply):
        if hasattr(self, 'f'):
            self.f.close()
        data_path = await self.config_request_value('global.paths.data_path')
        assert data_path
        pass_id = await self.config_request_value('global.mission.pass_id')
        assert pass_id is not None
        sv_name = await self.config_request_value('instance.space_vehicle.name')
        assert sv_name
