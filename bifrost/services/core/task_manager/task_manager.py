...
from bifrost.services.core.task_manager.task_types import Tasks, Task_Transformers  # noqa # Task_Transformers is used dynamically
...
class Task_Manager(Service):
    ...
    async def reconfigure(self, topic, message, reply):
        ...
        self.pass_number = await self.config_request("global.mission.pass_id")
        self.sv_name = await self.config_request('instance.space_vehicle.sv_name')
        self.aws_region = await self.config_request('global.aws.region')
        self.aws_bucket = await self.config_request('global.aws.bucket')
        self.aws_profile = await self.config_request('global.aws.profile')
    ...
    async def write_index(self, skip_s3=False):
        ...
        index_filepath = Path(downlink_path / "file_downlink" / f"downlink_index_{self.sv_name}_{self.pass_number}.json")
        ...
        s3_path = f"data/{self.pass_number}/{self.sv_name}/file_downlink/{filename}"
    ...
    async def track_file_reassembly(self, completed_task):
        if completed_task.md5_file:
            self.completed_file_reassembly['md5_pass'][completed_task.ground_tag].append(completed_task.subset_map())
        else:
            self.completed_file_reassembly['md5_fail'][completed_task.ground_tag].append(completed_task.subset_map())
            ...
            try:
                ...
                if new_task:
                    if isinstance(new_task, list):
                        for i in new_task:
                            await self.publish(i.name, i)
                    else:
                        await self.publish(new_task.name, new_task)
            except Exception as e:
    ...
    async def notify_pubsub(self, msg_type, completed_task):
        ...
        msg = {'task_id': completed_task.ID,
               ...
               'result': completed_task.subset_map()
               }
